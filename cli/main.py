...
def init(endpoint, contracts_url, wallet):
    safe_mk_dirs(SKALE_ALLOCATOR_CONFIG_FOLDER)
    download_file(contracts_url, SKALE_ALLOCATOR_ABI_FILE)
    config = {
        'endpoint': endpoint.strip(),
        'wallet': wallet
    }
    write_json(SKALE_ALLOCATOR_CONFIG_FILE, config)
...
if __name__ == '__main__':
    ...
    cmd_collection = click.CommandCollection(sources=[cli, sgx_cli, escrow_cli, wallet_cli])
    try:
        cmd_collection()
